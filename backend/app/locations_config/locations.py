...
# Main city for each županija
ZUPANIJA_CAPITALS: Dict[str, str] = {
...
def get_location_context(zupanija: str) -> Dict:
    """
    Get location context for LLM classification.
    ...
    Returns:
        Dict with:
        - zupanija: Full županija name
        - city: Main city (capital)
        - districts: List of all districts including the main city
    """
    city = ZUPANIJA_CAPITALS.get(zupanija, "")
    districts = ZUPANIJE.get(zupanija, [])
    
    # Include all districts - the main city center is also a valid district value
    # Clean up "X - Okolica" entries to just use the base name
    cleaned_districts = []
    for d in districts:
        if " - Okolica" in d:
            cleaned_districts.append(d.replace(" - Okolica", ""))
        else:
            cleaned_districts.append(d)
    
    # Remove duplicates and sort
    cleaned_districts = sorted(list(set(cleaned_districts)))
    
    return {
        "zupanija": f"{zupanija} županija",
        "city": city,
